                                     ...
                                     esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT:
        //d_addr = param->client_recv_publish_msg.ctx->addr;
        //ESP_LOGI("", "received publish message 0x%06x", param->client_recv_publish_msg.opcode);
        p = (uint8_t *)param->client_recv_publish_msg.msg;
        //ESP_LOG_BUFFER_HEX("",p,sizeof(p));
        d_addr = *(uint16_t *)p;
        if (d_addr != self_addr)
        {
