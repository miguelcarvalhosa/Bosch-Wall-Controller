                                              ...
                                              esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND: {
            ...
            set_state.model_pub_set.element_addr = 0x0001;
            set_state.model_pub_set.publish_app_idx = prov_key.app_idx;
            set_state.model_pub_set.publish_addr = 0xC000;
            set_state.model_pub_set.cred_flag = false;
            set_state.model_pub_set.publish_ttl = 127;
            set_state.model_pub_set.publish_period = 30;
            set_state.model_pub_set.publish_retransmit = 20;
            set_state.model_pub_set.model_id = ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_CLI;
            set_state.model_pub_set.company_id = CID_NVAL; 

            
            err = esp_ble_mesh_config_client_set_state(&common, &set_state);
            if (err) {
                ESP_LOGE(TAG, "%s: Config Model Pub Set failed", __func__);
                return;
            }
            uint8_t stat_pub = param->status_cb.model_pub_status.status;
            ESP_LOGI(TAG,"PUBLICATION STATUS: %d",stat_pub);
            ESP_LOGI(TAG, "elem_addr 0x%04x, pub_addr 0x%04x, period 0x%04x, mod_id 0x%04x",
                param->status_cb.model_pub_status.element_addr,
                param->status_cb.model_pub_status.publish_addr,
                param->status_cb.model_pub_status.period,
                param->status_cb.model_pub_status.model_id);
            ...
            /*ESP_LOGI(TAG,"Publish cfg client: %d",config_client.publish_status);*/
            break;
        }
        case ESP_BLE_MESH_MODEL_OP_MODEL_PUB_SET: {
            ...
            esp_ble_mesh_cfg_client_set_state_t set_state = {0};
            set_state.model_sub_add.element_addr = 0x0005;
            ...
            /* nothing if added subscription to an address which was not a group address */
            err = esp_ble_mesh_config_client_set_state(&common, &set_state);
            if (err) {
                ESP_LOGE(TAG, "%s: Config Model Sub add failed", __func__);
                return;
            }
            uint8_t stat_sub = param->status_cb.model_sub_status.status;
            ESP_LOGI(TAG,"SUBSCRIPTION STATUS: %d",stat_sub);
            ESP_LOGI(TAG, "elem_addr 0x%04x, sub_addr 0x%04x, mod_id 0x%04x",
